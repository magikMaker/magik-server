    ...
    function requestListener(request, response) {
        ...
        function sendFileNotFound(responseObject) {
            ...
            for(var i = 0, l = responseObject.config.extensions.length; i < l + 1; ++i) {
                ...
                notFoundPath = path.normalize(responseObject.documentRoot + path.sep + config.notFound + extension);

                if(fs.existsSync(notFoundPath) && fs.statSync(notFoundPath).isFile()) {
                    responseObject.filePath = notFoundPath;
                    responseObject.fileName = path.normalize(path.sep + responseObject.config.notFound + extension);
                    break;
                }
            }
        ...
        function sendFile(error, responseObject) {
            ...
            function send(responseObject){
                ...
                // console.log('responseObject', bytes(responseObject.headers['Content-Length']), responseObject.counter, responseObject.filePath, responseObject.headers['Content-Type']);
